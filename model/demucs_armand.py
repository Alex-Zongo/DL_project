...
class PermuteWrapper(nn.Module):
    def __init__(self, child):
        super(PermuteWrapper, self).__init__()
        self.child = child
...
class PermuteWrapperWithSameShape(PermuteWrapper, WithSameShape):
    pass


# class MyTransformerLayer(WithSameShape, PermuteWrapper):
#     def __init__(self, )
...
class ArmandDemucs(nn.Module):
    def __init__(self, n_in, channel_features, n_out, target_output_size, num_levels, separate, instruments):
        ...
        for instrument in model_list:
            ...
            # )
            module.bottleneck = PermuteWrapperWithSameShape(
                nn.Sequential(
                    PositionalEncoding(channel_features[-1]),
                    TransformerEncoderLayer(channel_features[-1], nhead=4, hidden_size=channel_features[-1], dropout=0.5),
                    TransformerEncoderLayer(channel_features[-1], nhead=4, hidden_size=channel_features[-1], dropout=0.5),
                )
            )
    ...
    def check_padding_for_bottleneck(self, bottleneck, target_output_size):
        ...
        try:
            ...
            curr_size = module.bottleneck.get_input_size(curr_size)
