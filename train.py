...
from model.wave_u_net import Waveunet
...
if __name__ == '__main__':
    ...
    levels = 2  # number of DS/US blocks
    ...
    sample_freq = 20  # Write an audio summary into Tensorboard logs every X training iterations
    ...
    # MODEL
    model = Waveunet(channels, num_features, channels, instruments, kernel_size, target_output_size=target_outputs,
                     conv_type=conv_type, depth=depth, strides=strides, res=res, separate=separate)
    model = model.to(device)

    log_dir = './runs/waveunet'
    ...
    if is_load_model is not None:
        ...
        state = load_model(model, optimizer, "checkpoints/waveunet/checkpoint_2625", device)
...
# TODO
    ...
    # Mir_eval metrics
    test_metrics = evaluate(channels, dataset["test"], model, instruments)
